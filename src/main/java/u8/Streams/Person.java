...
public class Person {
    ...
    public static void main(String[] args) {
        Person[] persons = {new Person("Иван", 25, Position.DIRECTOR, 60000f),
                new Person("Николай", 27, Position.ENGINEER, 55000.25f),
                new Person("Олег", 29, Position.ENGINEER, 87123.55f),
                new Person("Пётр", 34, Position.ENGINEER, 33000.33f),
                new Person("John Smith", 42, Position.ENGINEER, 102000.89f),
                new Person("Володя", 24, Position.MANAGER, 37000.24f)
        };

        String[] str = {"ab", "два", "три", "четыре", "три", "один", "четыре", "четыре", "два", "три", "четыре", "ab"};

        findAndSortWords2(str);
    }
...
//        return Stream.of(people).collect(Collectors.averagingDouble(Person::getSalary));
    ...
    public static String oldestNMy(Person[] persons, int n) {
        String str = Stream.of(persons).sorted((a, b) -> {
            if (a.getAge() > b.getAge()) return -1;
            if (a.getAge() < b.getAge()) return +1;
            else return 0;
        }).limit(n).map(Person::getName).reduce("", (a, b) -> a + b + " ");
